...
```{r}
dest_delays <-left_join(avg_arrival_delay, airports, by = c("dest" = "faa"))
dest_delays
```
...
```{r}
dest_delays %>%
slice_max(avg_arrival_delays, n = 1)
```
...
```{r}
avg_departure_delay <- my_flights %>%
  group_by(carrier) %>%
  summarize(avg_departure_delays = mean(dep_delay, na.rm = TRUE)) 

airline_delays <- left_join(avg_departure_delay, airlines, by = "carrier")
...
```{r}
airline_delays %>%
  slice_min(avg_departure_delays, n = 1)
